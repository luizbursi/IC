...
#código para a extração de dados de cada imagem da base de dados BRODATZ
...
path_brodatz = "C:/Users/oluiz/OneDrive/Documentos/IC/Bases/brodatz/"

#cria um vetor com os arquivos da base de dados 
dirs_brodatz = [file for file in os.listdir(path_brodatz) if file.endswith('.png')]
...
typefile_f = "_brodatzFEAS.pkl"
typefile_c = "_brodatzCLASS.pkl"
...
for descriptor in dirs_descriptors:     #ele vai fazer o laço no diretório com todas as funções extratoras de features
    ...
    for images in dirs_brodatz:  #laço que percorre as imagens dentro da base
        try:   
            img = cv2.imread(path_brodatz + images)  #parte de processamento de imagens
